...
function attachDragPickerWithInertia(wheel) {
  ...
  // ----- 터치 드래그 -----
  wheel.addEventListener("touchstart", (e) => onDown(e.touches[0].clientY), {
    passive: true,
  });
  wheel.addEventListener("touchmove", (e) => onMove(e.touches[0].clientY), {
    passive: true,
  });
...
function init() {
  ...
  yearWheel.addEventListener("scroll", () => handleScroll(yearWheel), {
    passive: true,
  });
  monthWheel.addEventListener("scroll", () => handleScroll(monthWheel), {
    passive: true,
  });
  dayWheel.addEventListener("scroll", () => handleScroll(dayWheel), {
    passive: true,
  });
...
function handleSubmit() {
  ...
  alert(
    `선택값: ${selectedYear}년 ${selectedMonth}월 ${selectedDay}일 , ${gender}`,
  );

  location.href = "/html/page/artistselect.html";
}
