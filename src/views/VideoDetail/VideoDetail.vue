...
async function getVideo() {
    const data = await $api(`${process.env.VUE_APP_BASE_URL}/test/api/auth/videos.php`, 'GET', { id });
    result.value = data;
}

async function comment_ADD() {
    const data = await $api(`${process.env.VUE_APP_BASE_URL}/test/api/auth/comment_insert.php`, 'POST', {
        post_id: id,
...
async function getComments() {
    const data = await $api(`${process.env.VUE_APP_BASE_URL}/test/api/auth/comment_list.php?post_id=${id}`, 'GET');
    console.log(data);
