...
- 메인 페이지에서는 저장된 아티스트 정보를 기준으로 iTunes Search API를 호출해 곡 목록을 출력합니다.
...
- 플레이어와 미니플레이어가 같은 상태를 읽고 쓰며 커스텀 이벤트로 즉시 동기화합니다.
- 재생 위치 handoff, `previewUrl` 없는 트랙 스킵, autoplay 차단 대응을 포함합니다.
...
- 검색/차트 및 음악이 재생될떄마다 플레이리스트에 자동 등록이 됩니다.
